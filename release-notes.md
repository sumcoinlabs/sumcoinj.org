...
* SumcoinURI: Allow uppercase schema.
...
* Made the `SumcoinURI` class aware of native segwit addresses.
...
* Peer is now dropping Sumcoin Cash nodes early.
...
* The wallet protobuf has changed:
  * The outptScriptType of a `DeterministicKeyChain` is persisted. Chains in old protobufs are assumed to be of
    type `P2PKH`. As long as you don't use `P2WPKH` chains, you can switch back and forth sumcoinj 0.14 and 0.15.
  * The `TransactionSigner` message has been removed, as the entire concept of stateful transaction signers. If
    you haven't used stateful transaction signers, you can switch back and forth sumcoinj 0.14 and 0.15.
  * A new `ScriptWitness` message is stored with `TransactionInputs`. It's currently not terribly useful, since after
    braodcasting of transactions it isn't needed by sumcoinj and script execution isn't implemented for segwit yet.
...
* New method `AbstractSumcoinNetParams.isRewardHalvingPoint()` tells if a certain height is a reward halving point.
...
* New `MonetaryFormat()` constructor that uses the new Unicode Sumcoin symbol at code point `U+20BF`.
...
* The pull-tester tool (SumcoindComparisonTool) isn't packaged into pull-tests.jar any more.
...
* Improvements to message serialization:
  * A common `MessageSerializer` interface has been extracted from `SumcoinSerializer` so that other implementations can be plugged.
...
* The whole `org.sumcoinj.testing` package has been moved to the test classpath.
* A new `SumcoinURI.convertToSumcoinURI()` variant allows to generate URIs for non-Sumcoin networks.
* Wallet/SendRequest/Transaction improvements:
  * All wallet related classes have been moved to the `org.sumcoinj.wallet` package!
...
* Adjusts the minimum network fee to the one imposed by Sumcoin Core.
...
* The PeerGroup lock is no longer held whilst invoking Bloom filter providers: this helps you to avoid circular deadlock when calling back into sumcoinj from inside your provider.
...
* Thanks to Kosta Korenkov, we have an experimental multisig wallets implementation. Multisig (also "married") wallets are HD wallets that are connected to a third party risk analysis service or device. When married, the wallet tracks multiple BIP32 key trees, keeps them in sync and starts vending P2SH addresses.
  * As part of this work, transaction signing is now pluggable. TransactionSigner implementations can be added to the wallet and will be serialized into and out of the users saved wallet file. Signers are given a transaction to sign in sequence. This is intended for risk analysis providers to provide a class that talks to their server to get a signature of the right form, so that all sumcoinj based wallets can be easily upgraded to support the new provider.
* Reject messages are now deserialized and logged, though not yet exposed in the API.
* Upgraded to Guava 16 and Bouncy Castle 1.51. Thanks to Peter Dettman and the rest of the Bouncy Castle team, sumcoinj now uses deterministic ECDSA for signing and we're now using an accelerated secp256k1 implementation that exploits the special properties of this curve, for dramatically faster calculations.
...
* Thanks to Andreas Schildbach:
  ...
  * A new Coin class replaces usage of BigInteger for marking values that are quantities of sumcoin. Formatting has moved into the new MonetaryFormat class.
...
* Thanks to Richard Green, there is now a Python version of the ForwardingService program from the getting started tutorial. This shows how to use sumcoinj from Python using the Jython interpreter.
* sumcoinj now probes localhost for a Sumcoin node and automatically uses that instead of the P2P network, when present. This means any sumcoinj based app can be easily upgraded from SPV to full security just by running Core at the same time: no setup needed.
* Thanks to Michael Bumann, there are now more example apps showing how to use parts of the API.
* WalletTemplate/WalletAppKit improvements. WalletTemplate is a demo app that shows how to create a cross-platform GUI wallet with a modern style and 60fps animations. WalletAppKit is a very high level API for creating apps that have a Sumcoin wallet:
...
* A JavaScript tutorial has been added, showing how to use sumcoinj from this language. More tutorials in other languages will come in future.
...
* The package name has changed to org.sumcoinj and the core Maven artifact name is now "sumcoinj-core". You can auto-port most of your code by running `find . -name '*.java' | xargs sed -i .bak 's/com.google.sumcoin./org.sumcoinj./g'`
* Wallet.completeTx now throws more precise unchecked exceptions in edge cases, instead of IllegalArgumentException.
* The use of BigInteger to represent quantities of Sumcoin has been replaced with the more efficient, type safe and useful class Coin. Coin is mostly source compatible with BigInteger so you can probably just do a search and replace to update your codebase. Utils.bitcoinValueToFriendlyString and friends moved to CoinFormat.
* NetworkParameters.getProofOfWorkLimit was renamed to getMaxTarget for consistency with other Sumcoin codebases.
* The library no longer uses the misleading term "nanocoins" to mean satoshis (the old term predated the use of the word satoshi to describe the smallest possible amount of sumcoin).
...
* The subVer string sent by sumcoinj now has a lower cased first component.
...
* sumcoinj 0.11 will shortly be available via Maven Central. Please use the dependency verifier plugin and/or check the PGP signatures on the uploads, if you use this!
...
* Info on testing your apps, and how to use regtest mode to make a private Sumcoin network that allows you to mine blocks instantly.
* A reference table showing which API's implement which Sumcoin Improvement Proposals (BIPs).

Please note that as I am no longer employed by Google, after 0.11 signing the Google contributor license agreement will no longer be necessary. I look forward to welcoming contributions from Andreas Schildbach now this requirement has gone away. Also, in future I plan to re-namespace the library from com.google.sumcoin to org.sumcoinj - auto-migration scripts will be provided when this is done.
...
* Matt Corallo's bitcoind comparison tool was merged in. It runs a large set of regression tests that compares the behaviour of sumcoinj in full verification mode against bitcoind.
...
* Also thanks to Matt, Bloom filtering is now implemented and activated by default. When sumcoinj connects to a peer that supports Bloom filtering, only transactions relevant to the wallet will be downloaded which makes bandwidth usage scale with the size of your wallet, not global system activity. A configurable false positive ratio allows you to trade off bandwidth vs privacy. App developers don't need to do anything to take advantage of this, it is enabled automatically.
...
* Support testnet3
* Support sumcoin-qt compatible message signing and verification.
...
* Address.getParameters() and Address#getParametersFromAddress() let you figure out for what network the address is for (test, production, etc). SumcoinURI no longer requires a NetworkParameters for the same reason.
...
* A bunch of new APIs to make writing Sumcoin apps easier.
